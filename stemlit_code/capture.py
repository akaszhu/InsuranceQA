from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.chrome.service import Service
from webdriver_manager.chrome import ChromeDriverManager
import time
from PIL import Image
import os
import streamlit as st
def take_screenshots(base_url, login_url, username, password, urls, output_dir="screenshots"):
    # Initialize the WebDriver
    driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()))

    # Open the login URL
    driver.get(login_url)

    # Wait for the page to load
    time.sleep(3)

    # Find the username and password input fields and the login button
    username_field = driver.find_element(By.NAME, 'username')  # Adjust the selector as needed
    password_field = driver.find_element(By.NAME, 'password')  # Adjust the selector as needed
    login_button = driver.find_element(By.XPATH, '//button[@type="submit"]')  # Adjust the selector as needed

    # Enter the username and password
    username_field.send_keys(username)
    password_field.send_keys(password)

    # Submit the login form
    login_button.click()

    # Wait for the login process to complete
    time.sleep(5)

    # Dictionary to store the screenshot file paths
    screenshot_paths = {}

    # Ensure the output directory exists
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    # Loop through each URL and take a screenshot
    for url in urls:
        # Navigate to the target page
        driver.get(url)

        # Wait for the page to load
        time.sleep(3)

        # Take a screenshot
        screenshot_path = os.path.join(output_dir, f"{url.replace('https://', '').replace('/', '_')}.png")
        driver.save_screenshot(screenshot_path)
        st.image(screenshot_path)
        # Store the screenshot file path in the dictionary
        screenshot_paths[url] = screenshot_path

    # Close the WebDriver
    driver.quit()

    # Return the collected data
    return screenshot_paths


